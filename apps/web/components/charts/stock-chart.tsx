...
export function StockChart({
  ...
  // Hold Space to expand the hover tooltip to the rich layout;
  // default stays compact so it doesn't dominate the small card.
          ...
          <ResponsiveContainer width="100%" height={Math.round(height * (showVolume ? 0.78 : 1))}>
            ...
            >
              ...
              <Tooltip
                ...
                content={(p: unknown) => (
                  <ChartTooltip raw={p} variant={variant} expanded={tooltipExpanded} />
                )}
              />
...
function ChartTooltip({
  raw, variant, expanded,
}: { raw: unknown; variant: 'kr' | 'us'; expanded: boolean }) {
  ...
  if (!expanded) {
    return (
      <div className="rounded-md border border-border-default bg-bg-secondary/95 backdrop-blur-sm p-3 text-[13px] shadow-lg min-w-[210px]">
        <div className="text-txt-primary text-[14px] font-mono font-bold mb-2 pb-1.5 border-b border-border-default/50 tabular-nums">
          {row.date}
        </div>
        ...
        <div className="mt-2 pt-1.5 border-t border-border-subtle/40 text-[10px] text-txt-muted italic flex items-center gap-1">
  ...
  return (
    ...
    >
      ...
      {/* OHL grid — all values text-primary (black on light theme).
          Only the directional close + day change pill above use up/
          down colors. */}
      <div className="px-3.5 py-3 grid grid-cols-3 gap-x-3 gap-y-1">
        <CompactStat label="시가" value={fmt(row.open)} />
        <CompactStat label="고가" value={fmt(row.high)} />
        <CompactStat label="저가" value={fmt(row.low)} />
      </div>

      {/* Volume + MA — values in text-primary, MA swatches keep their
          chart-line color so users can identify which MA they're
          reading. */}
        ...
        <div className="px-3.5 py-2.5 bg-bg-secondary/30 border-t border-border-subtle/50 space-y-1.5">
          {row.volume > 0 && (
            <div className="flex items-center justify-between">
              ...
              <span className="font-mono font-bold text-[13px] tabular-nums text-txt-primary">
